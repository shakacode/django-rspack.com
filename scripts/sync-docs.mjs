...
function cloneRepo(repoUrl, ref) {
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "django-rspack-docs-"));
  try {
...
async function main() {
  const configuredRepo = process.env.DJANGO_RSPACK_REPO;
  const localRepo = configuredRepo
    ? path.resolve(configuredRepo)
    : path.resolve(workspaceRoot, "../django-rspack");
  ...
  if (!(await exists(localDocs))) {
    const repoUrl =
      process.env.DJANGO_RSPACK_REPO_URL ?? "https://github.com/shakacode/django-rspack.git";
    const ref = process.env.DJANGO_RSPACK_REF ?? "main";
