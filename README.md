...
Production site workspace for [django-rspack.com](https://django-rspack.com), built with Docusaurus.

## Architecture

- Canonical markdown source stays in `django-rspack/docs/`
- This repo syncs docs at build time into `prototypes/docusaurus/docs`
- Site-owned pages live here (landing page, resources, support page)
...
`scripts/sync-docs.mjs` resolves the source repo in this order:

1. `DJANGO_RSPACK_REPO` env var
2. sibling directory `../django-rspack`
3. shallow clone from `DJANGO_RSPACK_REPO_URL` (default: upstream GitHub repo)

## Cloudflare Pages

- Project: `django-rspack-com`
...
`shakacode/django-rspack` should include `.github/workflows/trigger-docs-site.yml`, which dispatches
`docs-updated` events to this repository when `docs/**` changes on `main`.

That source workflow requires these secrets in `shakacode/django-rspack`:
